    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        if (s < e) {
            if (get_hap_val_initial(s) == '0') {
                // Accessing by original hap_id 's'
                ps_holder[s] = 0;
            } else {
                ps_holder[s] = 1;
            }
            ...
            for (int k = s + 1; k < e; ++k) {
                // Accessing by original hap_id 'k'
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        if (s == 0) {
            ...
            if (min_one_search) {
                int seek_index = s - 1;
                while (seek_index >= 0 && get_hap_val(old_pda_parr[seek_index]) == '0') {
                    // C# map.Get_HapVal(oldPDA.pArr[seekIndex]) == '0'
                    seek_index--;
                }
                if (seek_index >= 0) {
    ...
    int L_long_match_min_len) // Program.LLM_Len
{
    ...
    for (int site_idx = 1; site_idx < n_sites; ++site_idx) {
        // site_idx is the current site for PBWT update
        // coreP_Arr
        core_p_arr(
        ...
        report_long_matches_sl(
            ...
            [&](int original_hap_id) { return get_hap_val_for_site_k(site_idx, original_hap_id); },
            // Alleles at current site_idx for mismatch
